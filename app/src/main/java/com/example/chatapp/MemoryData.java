...
public class MemoryData {
    ...
    public static void saveLastmsg(String data,String chatKey, Context context){
        try
        {
            FileOutputStream fileOutputStream = context.openFileOutput(chatKey+"chatkey.txt",Context.MODE_PRIVATE);
            fileOutputStream.write(data.getBytes());
            fileOutputStream.close();
    ...
    public static String getLastmsg(Context context,String chatKey) {
        String data = "0";
        try {
            FileInputStream fis= context.openFileInput(chatKey+"chatkey.txt");
            InputStreamReader isr = new InputStreamReader(fis);
            BufferedReader bufferedReader =new BufferedReader(isr);
            StringBuilder sb = new StringBuilder();
            String line;
